...
export const requests: readonly ItemRequestJSON[] = (
  [
    {
      description:
  ...
  ] satisfies readonly Omit<ItemRequestJSON, "nameHTML" | "requesterHTML">[]
)
  .map((item) => {
    ;(item as ItemRequestJSON).nameHTML = escapeHTML(item.name)
    ;(item as ItemRequestJSON).requesterHTML = escapeHTML(item.requester)

    return item as ItemRequestJSON
  })
  ...
  .sort((a, b) =>
    a.requester < b.requester ? -1 : a.requester > b.requester ? 1 : 0
  )
  .sort((a, b) =>
    a.requester.toLowerCase() < b.requester.toLowerCase()
      ? -1
      : a.requester.toLowerCase() > b.requester.toLowerCase()
      ? 1
      : 0
